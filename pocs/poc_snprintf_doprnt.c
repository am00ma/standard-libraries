        ...
        else /* We got a format specifier! */
        {
            ...
            *sptr++ = *ptr++; /* Copy the % and move forward.  */
            ...
            if (*ptr == '*') COPY_VA_INT;
                ...
                while (isdigit(*ptr)) /* Handle explicit numeric value.  NOTE: Changed `ISDIGIT` to `isdigit` */
                    *sptr++ = *ptr++;
            ...
            while (strchr("hlL", *ptr))
            {
                switch (*ptr)
                {
                case 'h': short_width = 1; break;
                case 'l': wide_width++; break;
                case 'L': wide_width = 2; break;
                default: abort();
                }
                *sptr++ = *ptr++;
            }
            ...
            case 'c': {
                /* Short values are promoted to int, so just copy it
                           ...
                           to the right width.  */
                if (short_width) PRINT_TYPE(int);
                else
                {
                    switch (wide_width)
                    {
                    case 0: PRINT_TYPE(int); break;
                    case 1: PRINT_TYPE(long); break;
                    case 2:
                    default:
            ...
            case 'g':
            case 'G': {
                if (wide_width == 0) PRINT_TYPE(double);
                else
                {
            ...
            break;
            case 's': PRINT_TYPE(char*); break;
            case 'p': PRINT_TYPE(void*); break;
...
static int checkit(const char* format, ...)
{
    int     result;
    va_list args;

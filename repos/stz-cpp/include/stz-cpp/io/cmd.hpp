    ...
    isize err_maxlen)
{
    ...
    { // Parent process
        ...
        Buf bout(a, out_maxlen, AllocFlags::NOZERO);
        cap       = bout.cap;
        num_bytes = 0;
        do
        {
            if (cap <= 0) return {ENOMEM, {}};
            num_bytes  = read(stdout_pipe[0], bout.buf + bout.len, static_cast<usize>(cap));
            bout.len  += num_bytes;
            cap       -= num_bytes;
        } while (num_bytes != 0);
        bout.Shrink(a);
        ...
        // TODO: Not reading out and error simultaneously
        Buf berr(a, err_maxlen, AllocFlags::NOZERO);
        cap       = berr.cap;
        num_bytes = 0;
        do
        {
            if (cap <= 0) return {ENOMEM, {}};
            num_bytes  = read(stderr_pipe[0], berr.buf + berr.len, static_cast<usize>(cap));
            berr.len  += num_bytes;
            cap       -= num_bytes;
        } while (num_bytes != 0);
        berr.Shrink(a);
